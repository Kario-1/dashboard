...
function formatModelResponse(message) {
    ...
    // ممكن تضيف معالجات تانية لعلامات مشهورة لو لاحظتها في ردود الموديل
...
function appendMessage(sender, message) {
    ...
    messageElement.innerHTML = `<strong>${sender === 'user' ? 'أنت' : 'الروبوت'}:</strong> ${displayedMessage}`;
    ...
    try {
        ...
        if (chatData.messages && chatData.messages.length > 0) {
            ...
            chatData.messages.forEach(msg => {
                ...
                // هنا بنفترض أن الـ API بيرجع الـ sender 'user' أو 'bot'
                appendMessage(msg.role || 'bot', msg.content);
            });
        } else {
